...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    if (empty($userErrors)) {
        try {
            ...
            $stmt = $conn->prepare("UPDATE edu_user SET username=?,firstName=?,lastName=?,email=?,driverId=?,phone_number=?,address=?,city=?,state=?,country=?,zipcode=?,userType=?,status=?,organization_id=?,organization_name=? WHERE edu_user_id=?");
