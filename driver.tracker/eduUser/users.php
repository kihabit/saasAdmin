...
try {
    ...
    $countSql = "SELECT COUNT(*) as total FROM edu_user $whereClause";
    if ($whereClause) {
    ...
    $statsSql = "SELECT COUNT(*) as total_active, COUNT(CASE WHEN driverId IS NOT NULL AND driverId != '' THEN 1 END) as active_drivers, COUNT(CASE WHEN created_at IS NOT NULL THEN 1 END) as recent_logins FROM edu_user WHERE status = 1";
