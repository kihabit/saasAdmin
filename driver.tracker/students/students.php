...
if (isset($_POST['delete_child'], $_POST['child_id'])) {
    $deleteChildId = intval($_POST['child_id']);
    try {
        $conn->begin_transaction();
        $stmt = $conn->prepare("SELECT name, roll_number FROM students WHERE id = ?");
        $stmt->bind_param("i", $deleteChildId); $stmt->execute();
        $deletedChild = $stmt->get_result()->fetch_assoc(); $stmt->close();

        if ($deletedChild) {
            $stmt = $conn->prepare("DELETE FROM students WHERE id = ?");
            $stmt->bind_param("i", $deleteChildId); $stmt->execute(); $stmt->close();
            $conn->commit();
            ...
            $_SESSION['message'] = 'Child record not found.'; $_SESSION['messageType'] = 'error';
        }
    } catch (Exception $e) {
        $conn->rollback();
...
try {
    ...
    if ($whereClause) {
        ...
        $totalChildren = $conn->query($countSql)->fetch_assoc()['total'];
    }
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Card */
        .students-card { background: white; border-radius: 14px; border: 1px solid #e2e8f0; overflow: hidden; margin-bottom: 1.5rem; }
        ...
        .count-badge { background: #0000FF; color: white; padding: 3px 10px; border-radius: 20px; font-size: .78rem; font-weight: 600; }

        /* Table */
        .students-table { width: 100%; border-collapse: collapse; }
        .students-table th, .students-table td { padding: 13px 16px; text-align: left; border-bottom: 1px solid #e2e8f0; }
        .students-table th { background: #f8fafc; font-weight: 600; color: #4a5568; font-size: .78rem; text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap; }
        .students-table td { font-size: .9rem; }
        .students-table tbody tr:hover { background: #f8fafc; }

        .child-info { display: flex; align-items: center; gap: 10px; }
        .child-avatar { width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #0000FF, #4169E1); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 12px; flex-shrink: 0; overflow: hidden; }
        ...
        .status-badge-child { padding: 4px 10px; border-radius: 20px; font-size: .73rem; font-weight: 600; display: inline-flex; align-items: center; gap: 4px; }
        ...
        .btn-sm { padding: 5px 10px; border-radius: 7px; font-size: .75rem; border: none; cursor: pointer; transition: all 0.25s ease; text-decoration: none; display: inline-flex; align-items: center; gap: 4px; font-weight: 500; white-space: nowrap; font-family: inherit; }
        ...
        .child-highlight { background: #f3f4f6; padding: .85rem 1rem; border-radius: 8px; margin: .85rem 0; border-left: 4px solid #dc2626; font-size: .88rem; }
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                ...
                <div class="page-actions">
                    ...
                    <a href="<?php echo BASE_URL; ?>students/add_students.php" class="btn-primary">
            ...
            <div class="students-card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-child"></i> All Students</h3>
                ...
                <table class="students-table">
                    ...
                    <tr>
                        ...
                        <td style="max-width:180px; font-size:.82rem; color:#4a5568;">
                ...
                <div class="empty-state">
                    ...
                    <?php if (!empty($search)): ?>
                        ...
                        <a href="students.php" class="btn-primary" style="margin-top:1rem;display:inline-flex;">
                            <i class="fas fa-arrow-left"></i> Show All Students
                        </a>
                    <?php else: ?>
                        <p>No students have been registered yet.</p>
                        <a href="<?php echo BASE_URL; ?>students/add_students.php" class="btn-primary" style="margin-top:1rem;display:inline-flex;">
            ...
            <div class="stats-summary">
                <div class="stat-item">
                    <i class="fas fa-child"></i>
                    <div><h4><?php echo number_format($totalChildren); ?></h4><p>Total Students</p></div>
                </div>
                <div class="stat-item">
                    <i class="fas fa-check-circle"></i>
                    <div><h4><?php echo number_format($totalActive); ?></h4><p>Active Students</p></div>
                </div>
                <div class="stat-item">
                    <i class="fas fa-user-friends"></i>
                    <div><h4><?php echo number_format($parentCount); ?></h4><p>Parents with Student</p></div>
                </div>
                <div class="stat-item">
                    <i class="fas fa-database"></i>
                    <div><h4><?php echo number_format($totalChildren - $totalActive); ?></h4><p>Inactive Students</p></div>
                </div>
            </div>
