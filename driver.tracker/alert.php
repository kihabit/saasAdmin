...
if (isset($_POST['delete_alert'], $_POST['alert_id'])) {
    $deleteAlertId = intval($_POST['alert_id']);
    try {
        $stmt = $conn->prepare("DELETE FROM alerts WHERE id = ?");
        $stmt->bind_param("i", $deleteAlertId); $stmt->execute();
...
try {
    ...
    if ($whereClause) {
        ...
        $totalAlerts = $stmt->get_result()->fetch_assoc()['total']; $stmt->close();
    } else {
        $totalAlerts = $conn->query($countSql)->fetch_assoc()['total'];
    }
    ...
    if ($whereClause) {
        ...
        $stmt = $conn->prepare($sql); $stmt->bind_param('ii', $limit, $offset);
    }
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .sidebar-user{margin-top:1rem;padding:1rem;background:rgba(255,255,255,.1);border-radius:12px;backdrop-filter:blur(10px)}
        ...
        .logout-btn{background:#dc3545;color:white;border:none;padding:7px 14px;border-radius:8px;font-weight:500;font-size:.85rem;cursor:pointer;transition:all .3s;text-decoration:none;display:flex;align-items:center;gap:6px}
        ...
        .search-input{width:100%;padding:9px 14px 9px 38px;border:1px solid #e2e8f0;border-radius:10px;background:white;font-size:.88rem;transition:all .3s;font-family:inherit}
        .search-input:focus{outline:none;border-color:#0000FF;box-shadow:0 0 0 3px rgba(0,0,255,.1)}
        .search-icon{position:absolute;left:13px;top:50%;transform:translateY(-50%);color:#9ca3af;font-size:.8rem}

        /* Message */
        .message-container{margin-bottom:1.25rem}
        .message{padding:.85rem 1.25rem;border-radius:10px;display:flex;align-items:center;gap:10px;font-weight:500;font-size:.9rem;animation:slideIn .3s ease}
        .message.success{background:#d1fae5;color:#065f46;border:1px solid #a7f3d0}
        .message.error{background:#fee2e2;color:#991b1b;border:1px solid #fca5a5}

        /* Card */
        .alerts-card{background:white;border-radius:14px;border:1px solid #e2e8f0;overflow:hidden;margin-bottom:1.5rem}
        ...
        .alerts-count{background:#0000FF;color:white;padding:3px 10px;border-radius:20px;font-size:.78rem;font-weight:600}

        /* Table */
        .alerts-table{width:100%;border-collapse:collapse}
        .alerts-table th,.alerts-table td{padding:13px 16px;text-align:left;border-bottom:1px solid #e2e8f0}
        .alerts-table th{background:#f8fafc;font-weight:600;color:#4a5568;font-size:.78rem;text-transform:uppercase;letter-spacing:.5px;white-space:nowrap}
        .alerts-table td{font-size:.9rem}
        .alerts-table tbody tr:hover{background:#f8fafc}

        /* Type badge */
        .type-badge{padding:4px 10px;border-radius:20px;font-size:.73rem;font-weight:600;display:inline-flex;align-items:center;gap:4px}
        ...
        .date-text{color:#4a5568;font-size:.82rem;white-space:nowrap}
        .text-muted{color:#9ca3af}
        .coord-text{font-size:.8rem;color:#4a5568;font-family:monospace}
        .message-text{max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.88rem;color:#374151}

        /* Action buttons */
        .actions{display:flex;gap:5px;align-items:center}
        .btn-sm{padding:5px 10px;border-radius:7px;font-size:.75rem;border:none;cursor:pointer;transition:all .25s;text-decoration:none;display:inline-flex;align-items:center;gap:4px;font-weight:500;white-space:nowrap;font-family:inherit}
        ...
        .modal{background:white;border-radius:16px;padding:2rem;max-width:460px;width:90%;box-shadow:0 20px 25px -5px rgba(0,0,0,.1);animation:scaleIn .3s ease}
        .modal-header{display:flex;align-items:center;gap:12px;margin-bottom:1.25rem}
        .modal-icon{width:44px;height:44px;border-radius:50%;background:#fee2e2;color:#dc2626;display:flex;align-items:center;justify-content:center;font-size:1.3rem}
        .modal-header h3{font-size:1.25rem;font-weight:600;color:#1a202c}
        .modal-body{margin-bottom:1.5rem;color:#4a5568;line-height:1.6;font-size:.9rem}
        .alert-highlight{background:#f3f4f6;padding:.85rem 1rem;border-radius:8px;margin:.75rem 0;border-left:4px solid #dc2626;font-size:.88rem}
        ...
        /* View modal */
        .view-detail{display:flex;flex-direction:column;gap:.7rem}
        .view-row{display:flex;gap:1rem;border-bottom:1px solid #e2e8f0;padding-bottom:.7rem}
        .view-label{font-weight:600;color:#4a5568;min-width:105px;font-size:.85rem}
        .view-value{color:#1a202c;font-size:.85rem;word-break:break-all}

        /* Pagination */
        .pagination{display:flex;justify-content:center;align-items:center;gap:6px;margin-top:1.5rem}
        .pagination a,.pagination span{padding:7px 11px;border-radius:8px;text-decoration:none;color:#4a5568;font-weight:500;font-size:.88rem;transition:all .3s}
        ...
        .empty-state h3{font-size:1.1rem;margin-bottom:.4rem;color:#4a5568}
        .empty-state p{font-size:.88rem}

        /* Stats */
        .stats-summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:1.25rem;margin-top:1.5rem}
        .stat-item{background:white;padding:1.25rem;border-radius:12px;border:1px solid #e2e8f0;display:flex;align-items:center;gap:1rem;transition:all .3s}
        .stat-item:hover{transform:translateY(-2px);box-shadow:0 4px 12px rgba(0,0,0,.1)}
        .stat-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.2rem;color:white;flex-shrink:0}
        .stat-item h4{font-size:1.35rem;font-weight:700;color:#1a202c;margin-bottom:2px}
        .stat-item p{color:#718096;font-size:.82rem;margin:0}
        ...
        @media(max-width:1024px){
            ...
            .page-actions{flex-direction:column}.search-box{min-width:auto}
        }
        @media(max-width:768px){
            .header,.main-content{padding:.75rem 1rem}
            .alerts-table th,.alerts-table td{padding:10px 8px}
            .stats-summary{grid-template-columns:1fr 1fr;margin-top:1rem}
        }
        ...
        <main class="main-content">
            ...
            <div class="alerts-card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-bell"></i> All Alerts</h3>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="type-badge <?php echo $td['class']; ?>">
                                <i class="fas <?php echo $td['icon']; ?>"></i> <?php echo $td['label']; ?>
                            </span>
                        </td>
                        <td>
                        ...
                        <td class="coord-text"><?php echo !empty($alert['latitude'])  ? htmlspecialchars($alert['latitude'])  : '<span class="text-muted">—</span>'; ?></td>
                        <td class="coord-text"><?php echo !empty($alert['longitude']) ? htmlspecialchars($alert['longitude']) : '<span class="text-muted">—</span>'; ?></td>
                        <td class="date-text"><?php echo !empty($alert['created_at']) ? date('M j, Y g:i A', strtotime($alert['created_at'])) : '—'; ?></td>
                        <td>
                            <div class="actions">
                ...
                <div class="empty-state">
                    ...
                    <?php if (!empty($search)): ?>
                        ...
                        <a href="alert.php" style="margin-top:1rem;display:inline-flex;align-items:center;gap:8px;background:#0000FF;color:white;padding:9px 16px;border-radius:10px;text-decoration:none;font-weight:600;font-size:.85rem;">
            ...
            <div class="pagination">
                ...
                if($s>1){ echo "<a href='?page=1$qs'>1</a>"; if($s>2) echo "<span>…</span>"; }
                for($i=$s;$i<=$e;$i++) echo $i==$page ? "<span class='current'>$i</span>" : "<a href='?page=$i$qs'>$i</a>";
                if($e<$totalPages){ if($e<$totalPages-1) echo "<span>…</span>"; echo "<a href='?page=$totalPages$qs'>$totalPages</a>"; }
            ...
            <div class="stats-summary">
                <div class="stat-item">
                    <div class="stat-icon" style="background:linear-gradient(135deg,#0000FF,#4169E1)"><i class="fas fa-bell"></i></div>
                    <div><h4><?php echo number_format($statsData['total'] ?? 0); ?></h4><p>Total Alerts</p></div>
                </div>
                <div class="stat-item">
                    <div class="stat-icon" style="background:linear-gradient(135deg,#1e40af,#3b82f6)"><i class="fas fa-map-marker-alt"></i></div>
                    <div><h4><?php echo number_format($statsData['geo_fence'] ?? 0); ?></h4><p>Geo-Fence</p></div>
                </div>
                <div class="stat-item">
                    <div class="stat-icon" style="background:linear-gradient(135deg,#dc2626,#ef4444)"><i class="fas fa-exclamation-triangle"></i></div>
                    <div><h4><?php echo number_format($statsData['sos'] ?? 0); ?></h4><p>SOS Alerts</p></div>
                </div>
                <div class="stat-item">
                    <div class="stat-icon" style="background:linear-gradient(135deg,#b45309,#f59e0b)"><i class="fas fa-clock"></i></div>
                    <div><h4><?php echo number_format($statsData['delay'] ?? 0); ?></h4><p>Delay Alerts</p></div>
                </div>
                <div class="stat-item">
                    <div class="stat-icon" style="background:linear-gradient(135deg,#374151,#6b7280)"><i class="fas fa-info-circle"></i></div>
                    <div><h4><?php echo number_format($statsData['other'] ?? 0); ?></h4><p>Other</p></div>
                </div>
            </div>
...
<div class="modal-overlay" id="viewModal">
    <div class="modal" style="max-width:520px;">
        <div class="modal-header">
            <div class="modal-icon" style="background:#dbeafe;color:#1e40af;"><i class="fas fa-bell"></i></div>
            <h3>Alert Details</h3>
        </div>
        <div class="modal-body">
            <div class="view-detail">
                <div class="view-row"><span class="view-label">Alert ID</span><span class="view-value" id="viewId"></span></div>
                <div class="view-row"><span class="view-label">Type</span><span class="view-value" id="viewType"></span></div>
                <div class="view-row"><span class="view-label">Message</span><span class="view-value" id="viewMessage"></span></div>
                <div class="view-row"><span class="view-label">Latitude</span><span class="view-value" id="viewLatitude"></span></div>
                <div class="view-row"><span class="view-label">Longitude</span><span class="view-value" id="viewLongitude"></span></div>
                <div class="view-row" style="border:none;padding:0;"><span class="view-label">Created At</span><span class="view-value" id="viewCreatedAt"></span></div>
            </div>
        </div>
        <div class="modal-actions">
...
<script>
    ...
    function hideViewModal() { document.getElementById('viewModal').classList.remove('active'); }

    document.querySelectorAll('.modal-overlay').forEach(overlay => {
        overlay.addEventListener('click', function(e) { if(e.target===this) this.classList.remove('active'); });
    });
    document.addEventListener('keydown', e => { if(e.key==='Escape'){ hideDeleteModal(); hideViewModal(); }});
