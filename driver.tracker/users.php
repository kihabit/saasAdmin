...
if (isset($_POST['delete_user']) && isset($_POST['user_id'])) {
    ...
    if ($deleteUserId == $user_id) {
        ...
        try {
            $conn->begin_transaction();
            $stmt = $conn->prepare("SELECT username, email FROM user_login WHERE user_id = ?");
...
try {
    ...
    $statsSql = "SELECT COUNT(*) as total_active, COUNT(*) as active_drivers, COUNT(CASE WHEN last_login IS NOT NULL THEN 1 END) as recent_logins FROM user_login WHERE status = 1";
    ...
    $sql = "SELECT user_id, driverId, username, firstName, lastName, email, status, created_at, last_login, userType FROM user_login $whereClause ORDER BY created_at DESC LIMIT ? OFFSET ?";
    if ($whereClause) {
